...
SELECT DISTINCT s.PartDisplayName AS concept_name,
...
WHERE pl.LinkTypeName IN ('Primary')
  AND pl.PartTypeName IN ('SYSTEM', 'METHOD', 'PROPERTY', 'TIME', 'COMPONENT', 'SCALE')
AND pl.PartNumber NOT IN (SELECT DISTINCT concept_code FROM concept_stage WHERE concept_code IS NOT NULL)
AND p.Status = 'ACTIVE'
