...
join rx_combo using (i_combo)
join concept c on c.concept_id=drug_concept_id
where concept_code_1 not in (select concept_code_1 from atc_to_drug_2) and concept_code_1 not like '% %' --w/o forms
...
where h.concept_code_1 not in (select concept_code_1 from atc_to_drug_2))
select concept_code_1, atc_name, c.concept_id, c.concept_name, c.concept_class_id
from atc_comb
join rx_combo using (i_combo)
join concept c on c.concept_id=drug_concept_id
